...
```
 ...
 Для этого возьмите одно из спам-писем из датасета и внесите в него изменения, так чтобы фильтр его принял за хам. Функции из `email_read_util.py` умеют полноценно читать только письма, содержащие текстовый контент (`Content-Type: text/plain;`), поэтому модифицировать стоит письма именно с таким содержимым. 
